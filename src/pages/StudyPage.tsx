...
export default function StudyPage() {
  ...
  const [tab, setTab] = useState<'study' | 'flash'>('study');
  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-2xl mb-6 w-fit">
        {[
          { id: 'study' as const, label: '📖 오늘의 학습' },
          { id: 'flash' as const, label: '🃏 플래시카드' },
        ].map(t => (
          <button
            key={t.id}
            onClick={() => setTab(t.id)}
            className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${
              tab === t.id ? 'bg-white text-exam-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            {t.label}
          </button>
